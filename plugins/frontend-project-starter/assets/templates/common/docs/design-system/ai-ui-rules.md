...
- Treat `design-tokens.yaml` as a contract and audit input, not as a generated CSS source. Runtime variables must exist in the target project's theme chain.
...
- Run `python .agents/skills/frontend-design-system/scripts/validate_design_tokens.py . --all` when changing `design-tokens.yaml`, tenant overlays, or the schema.
- Run `python .agents/skills/frontend-design-system/scripts/audit_design_tokens.py . --paths <changed-runtime-files>` before finalizing design-sensitive runtime changes.
