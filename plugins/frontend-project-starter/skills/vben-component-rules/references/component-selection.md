...
This reference describes the default shared component contract for Vben projects that keep reusable UI under project component directories such as `src/components` or `apps/web-antd/src/components`. Keep this mapping in sync when project rules or shared components change.
...
| Timeline history | `Timeline` | Ad-hoc timeline markup |
...
## Retired Template Scaffolds

Do not import or reference retired local template scaffolds as runtime dependencies. If a pattern is needed, promote it into the project's real shared component directory first, then import the exported component.
...
| `Timeline` | `#/components/Timeline` | Timeline history display with grouped dates and optional attachments |
